...
export default function DashboardLayout() {
  ...
  // Force role to mentor-yashvi upon mount if not set to a mentor
  useEffect(() => {
    if (user?.role !== 'mentor') {
      const defaultMentor = users.find(u => u.role === 'mentor');
      if (defaultMentor) {
        switchRole('mentor');
      }
    }
  ...
  const menuItems = [
    ...
    { name: 'Grading Queue', path: '/grading', icon: FileCheck },
    { name: 'Session Manager', path: '/sessions', icon: CalendarDays },
    { name: 'Issue Badges', path: '/badges', icon: Award },
  ];
    ...
    <div className="flex h-screen bg-[#F8FAFC]">
      ...
      <aside className="w-64 bg-[#354E80] text-slate-100 flex flex-col shrink-0">
        ...
        <div className="px-6 py-2">
          <Badge variant="warning" className="text-[9px] py-1 font-extrabold tracking-widest bg-amber-500/10 border-amber-500/20 text-amber-500">
            MENTOR STAFF
          </Badge>
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-20 bg-white border-b border-slate-100 flex items-center justify-between px-8 z-10 shrink-0">
          <div className="w-96 relative">
            ...
            <input
              type="text"
              placeholder="Search active course classrooms..."
              className="w-full pl-10 pr-4 py-2 text-sm bg-slate-50 border border-slate-100 rounded-xl focus:outline-none focus:border-[#354E80] focus:ring-1 focus:ring-[#354E80] transition-all"
            />
          </div>
