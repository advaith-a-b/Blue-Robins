...
export default function DashboardLayout() {
  ...
  // Force role to parent-jain upon mount if not set to parent
  useEffect(() => {
    if (user?.role !== 'parent') {
      const defaultParent = users.find(u => u.role === 'parent');
      if (defaultParent) {
        switchRole('parent');
      }
    }
  ...
  const menuItems = [
    { name: 'Progress Dashboard', path: '/dashboard', icon: Home },
  ];
    ...
    <div className="flex h-screen bg-[#F8FAFC]">
      ...
      <aside className="w-64 bg-[#354E80] text-slate-100 flex flex-col shrink-0">
        ...
        <div className="px-6 py-2">
          <Badge variant="success" className="text-[9px] py-1 font-extrabold tracking-widest bg-emerald-500/10 border-emerald-500/20 text-emerald-400">
            PARENT PORTAL
          </Badge>
        </div>
